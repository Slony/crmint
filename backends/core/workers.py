...
class GAAudiencesUpdater(BQWorker, GAWorker):
  ...
  Visit https://bit.ly/2rnVpPz for more details on the required GA Audience JSON
  template format.
  """
...
class AdsAPISettingsBuilder(object):
  ...
  @staticmethod
  def build(params):
    """Creates Google Ads setting in YAML format."""

    settings = {
        'adwords': {
            'client_customer_id': params['client_customer_id'].strip(),
            'developer_token': params['developer_token'].strip(),
            'client_id': params['client_id'].strip(),
            'client_secret': params['client_secret'].strip(),
            'refresh_token': params['google_ads_refresh_token'].strip(),
        }
    }
    return yaml.dump(settings)


class BQToCMExporter(AdsAPIWorker, BQWorker):
  ...
  def _normalize_and_sha256(self, input_string):
    """Returns SHA-256 hash of a normalised version of an input string."""
  ...
  def _read_query_data(self, query_data, fields, worker_params):
    ...
    for row in query_data:
      ...
      if not customer_data.get(worker_params["list_name"]):
        ...
        customer_data[worker_params["list_name"]] = \
      ...
      if data.get(self.EMAIL):
        if self._params['encrypted_data']:
          ...
          list_data['emails'].append(
              {'hashedEmail': self._normalize_and_sha256(data[self.EMAIL])})
      ...
      if (data.get(self.FIRST_NAME) and data.get(self.LAST_NAME) and
        ...
        if self._params['encrypted_data']:
          ...
          address['hashedFirstName'] = \
              self._normalize_and_sha256(data[self.FIRST_NAME])
          address['hashedLastName'] = \
              self._normalize_and_sha256(data[self.LAST_NAME])
  ...
  def _upload_data(self, list_name, customer_data, is_remove_data):
    ...
    Args:
      ...
      is_remove_data: Whether remove (True) or add (False) user list items.
    """
    ads_api_client = self._get_ads_api_client()
    # Initialize appropriate services.
    user_list_service = ads_api_client.GetService('AdwordsUserListService',
                                                  'v201809')
    ...
    if result['entries']:
      ...
      user_list = {
          ...
          # to indicate unlimited; otherwise normal values apply.
    ...
    members = []
    for key in ['emails', 'emails', 'mobile_ids', 'user_ids', 'addresses']:
      if customer_data[key]:
        members.extend(customer_data[key])
  ...
  def _check_global_params(self):
    """Raise an exception if a global param is missing."""
    for key in self.GLOBAL_SETTINGS:
      try:
        assert self._params[key]
      except (KeyError, AssertionError):
        raise WorkerException('Global parameter "%s" is missing.' % key)
  ...
  def _process_query_results(self, query_data, query_schema):
    ...
    for row in query_data:
      ...
      if self.BODY_PARAM in data and data[self.BODY_PARAM] is not None:
        data = {self.BODY_PARAM: data[self.BODY_PARAM]}
      else:
        data = None
